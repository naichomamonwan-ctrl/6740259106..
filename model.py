...
# กำหนดค่าคงที่สำหรับชื่อฐานข้อมูล
DB_PATH = "library.db"

# ฟังก์ชันเชื่อมต่อฐานข้อมูล
def get_connection():
    """สร้างและคืนค่าการเชื่อมต่อกับฐานข้อมูล SQLite"""
    conn = sqlite3.connect(DB_PATH)
    return conn

# ---------------- ฟังก์ชันจัดการหนังสือ ----------------

def add_book_db(title: str, author: str):
    """เพิ่มหนังสือใหม่ลงฐานข้อมูล (CREATE)"""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
...
def get_all_books():
    """ดึงข้อมูลหนังสือทั้งหมดจากตาราง books แล้วคืนค่าเป็น DataFrame (READ)"""
    conn = get_connection()
...
def update_book_db(book_id: int, title: str, author: str):
    """อัปเดตข้อมูลหนังสือตาม id (UPDATE)"""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
...
def add_member_db(member_code: str, name: str, gender: str, email: str, phone: str, is_active: bool):
    """เพิ่มข้อมูลสมาชิกใหม่ลงฐานข้อมูล (CREATE)"""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        """
...
def is_member_code_exists(member_code: str):
    """ตรวจว่ารหัสสมาชิกนี้มีอยู่แล้วหรือยัง"""
    conn = get_connection()
    c = conn.cursor()
...
def is_email_exists(email: str):
    """ตรวจว่าอีเมลนี้ถูกใช้แล้วหรือยัง"""
    if not email:
        return False
    conn = get_connection()
    c = conn.cursor()
...
def get_all_members():
    """อ่านข้อมูลสมาชิกทั้งหมด และคืนค่าเป็น DataFrame (READ)"""
    conn = get_connection()
...
def delete_member_db(member_id: int):
    """ลบข้อมูลสมาชิกตาม id ที่ส่งมา (DELETE)"""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
...
def update_member_db(member_id: int, member_code: str, name: str, gender: str, email: str, phone: str, is_active: bool):
    """อัปเดตข้อมูลสมาชิกตาม id (UPDATE)"""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        """
